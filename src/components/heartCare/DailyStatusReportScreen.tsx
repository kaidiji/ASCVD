...
export const DailyStatusReportScreen: React.FC<Props> = ({
  ...
  // Today definition: Dynamic current date (normalized to 00:00:00)
  const TODAY = useMemo(() => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
  ...
  const todayDateStr = useMemo(() => getDateStr(TODAY), [TODAY]);

  // Helper to check if a date is in the future relative to TODAY
  ...
  // Currently selected date in calendar - defaults to TODAY when entering
  const [selectedDate, setSelectedDate] = useState<Date>(TODAY);

  // Top visible month tracking in scrollable calendar
  ...
  // Ref for continuous calendar scroll container
  const scrollContainerRef = React.useRef<HTMLDivElement>(null);

  // Generate 12 months for vertical continuous scroll (10 past months, current month, 1 future month)
  const monthsToRender = useMemo(() => {
    const list: Array<{ year: number; month: number; label: string }> = [];
    const currentY = TODAY.getFullYear();
    const currentM = TODAY.getMonth();

    // Past 10 months up to next month
  ...
  const [recordsMap, setRecordsMap] = useState<Record<string, DailyStatusRecord>>(() => {
    ...
    // Seed historical sample records for demonstration (e.g. August 17 and 18, 2026 as shown in screenshot)
    map['2026-08-17'] = {
      dateStr: '2026-08-17',
      formattedDisplayDate: '2026 年 8 月 17 日',
      slashDate: '2026/08/17',
      noSymptoms: false,
      selectedSymptomIds: ['fatigue'],
      symptomsDetails: {
        fatigue: createDefaultDetail('fatigue', '常覺得累、全身無力', 'yellow'),
      },
      feelingsText: '容易疲倦，稍微休息後有改善',
      reportedAt: '16:30',
    };
    ...
    map['2026-08-18'] = {
      ...
      noSymptoms: false,
      selectedSymptomIds: ['edema'],
      symptomsDetails: {
        edema: createDefaultDetail('edema', '下肢水腫（腳踝或小腿腫脹）', 'yellow'),
      },
      feelingsText: '腳踝稍有輕度水腫',
      reportedAt: '11:48',
    };
    ...
    if (heartCareState.reportedSymptoms && heartCareState.reportedSymptoms.length > 0) {
      ...
      map[todayDateStr] = {
        ...
        formattedDisplayDate: getFormattedDate(TODAY),
        slashDate: getSlashDate(TODAY),
        noSymptoms: false,
  ...
  // Directly position at today's date instantly when entering calendar view (no visible scroll animation)
  useLayoutEffect(() => {
    if (currentView === 'calendar') {
      const positionToday = () => {
        const todayMonthId = `month-${TODAY.getFullYear()}-${TODAY.getMonth()}`;
  ...
  const handleSaveRecord = () => {
    ...
    // Switch directly back to calendar/overview screen
  ...
  // =========================================================================
  // VIEW 1: CALENDAR OVERVIEW SCREEN (Reference: IMG_9041.PNG)
  // =========================================================================
  // Days of week header (Sun Mon Tue Wed Thu Fri Sat matching IMG_9041.PNG)
  ...
  // Scroll to a specific month or today
  const handleScrollToMonth = (year: number, month: number, smooth = true) => {
    setViewYear(year);
    setViewMonth(month);
    const targetId = `month-${year}-${month}`;
  ...
  const handleJumpToToday = () => {
    ...
    const todayEl = document.getElementById(`date-cell-${todayDateStr}`);
    if (todayEl) {
      todayEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } else {
      handleScrollToMonth(TODAY.getFullYear(), TODAY.getMonth(), true);
    }
  };

  // Handle scroll to sync month header with top visible month
  const handleContainerScroll = () => {
    if (isMonthPickerOpen) setIsMonthPickerOpen(false);
    if (!scrollContainerRef.current) return;
    const containerTop = scrollContainerRef.current.getBoundingClientRect().top;

    for (const mItem of monthsToRender) {
      const el = document.getElementById(`month-${mItem.year}-${mItem.month}`);
      if (el) {
        const rect = el.getBoundingClientRect();
        if (rect.bottom > containerTop + 30 && rect.top <= containerTop + 140) {
          setViewYear(mItem.year);
          setViewMonth(mItem.month);
          break;
        }
      }
    }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white text-slate-900 select-none font-sans overflow-hidden relative">
      ...
      {/* TOP HEADER (Reference: IMG_9041.PNG) */}
      <div className="px-4 py-3 bg-white flex items-center justify-between shrink-0 border-b border-slate-100 relative z-20">
        <button
          type="button"
          onClick={onBack}
          className="p-1 -ml-1 text-slate-600 hover:text-slate-900 active:scale-95 transition-all cursor-pointer"
      ...
      {/* STICKY MONTH SELECTOR BAR & TODAY BUTTON (Reference: IMG_9041.PNG) */}
      <div className="px-4 py-2.5 bg-white border-b border-slate-100 flex items-center justify-between shrink-0 relative z-20 shadow-2xs">
        {/* Month Dropdown Trigger */}
        <div className="relative">
          <button
            type="button"
            onClick={() => setIsMonthPickerOpen((prev) => !prev)}
            className="flex items-center gap-1.5 text-[1.125rem] font-extrabold text-slate-900 hover:text-[#ea580c] active:scale-98 transition-all cursor-pointer py-1 px-1 rounded-lg hover:bg-slate-50"
          >
            <span>
              {viewYear} 年 {viewMonth + 1}月
            </span>
            <ChevronDown className={`w-4 h-4 text-slate-700 transition-transform ${isMonthPickerOpen ? 'rotate-180 text-[#ea580c]' : ''}`} />
          </button>

          {/* Quick Month Dropdown Menu */}
          {isMonthPickerOpen && (
            <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 max-h-60 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-150">
              <div className="px-3 py-1 text-xs font-bold text-slate-400 uppercase tracking-wider">選擇月份</div>
              {monthsToRender.map((m) => (
                <button
                  key={`${m.year}-${m.month}`}
                  type="button"
                  onClick={() => {
                    handleScrollToMonth(m.year, m.month);
                    setIsMonthPickerOpen(false);
                  }}
                  className={`w-full text-left px-3 py-2 text-[0.9375rem] font-bold transition-colors cursor-pointer flex items-center justify-between ${
                    viewYear === m.year && viewMonth === m.month
                      ? 'bg-orange-50 text-[#ea580c]'
                      : 'text-slate-800 hover:bg-slate-50'
                  }`}
                >
                  <span>{m.label}</span>
                  {viewYear === m.year && viewMonth === m.month && <Check className="w-4 h-4 text-[#ea580c]" />}
                </button>
              ))}
            </div>
          )}
        ...
        <button
          ...
          className="text-[1.0625rem] font-extrabold text-[#ea580c] hover:text-[#c2410c] active:opacity-75 transition-all cursor-pointer px-3 py-1 rounded-lg hover:bg-orange-50"
        >
          今日
        </button>
      </div>

      {/* CONTINUOUS VERTICAL SCROLLING CALENDAR CONTAINER */}
      <div
        ref={scrollContainerRef}
        className="flex-1 overflow-y-auto px-4 py-2 pb-36 space-y-8"
        onScroll={handleContainerScroll}
      >
        {monthsToRender.map((mItem) => {
          const days = generateMonthDays(mItem.year, mItem.month);
          return (
            <div key={`${mItem.year}-${mItem.month}`} id={`month-${mItem.year}-${mItem.month}`} className="pt-2">
              {/* Month Section Header (e.g. 9月 2026 / 8月 2026) */}
              <div className="text-[1.125rem] font-black text-slate-900 px-1 mb-2.5">
                {mItem.month + 1}月 {mItem.year}
              </div>

              {/* Weekday Header Row: Sun Mon Tue Wed Thu Fri Sat */}
              <div className="grid grid-cols-7 text-center text-[0.9375rem] font-bold text-slate-900 mb-2 border-b border-slate-100 pb-2">
              ...
              <div className="grid grid-cols-7 gap-y-3.5 text-center items-center">
                {days.map((item, idx) => {
                  ...
                  const isFuture = isFutureDate(item.dateObj);
                  ...
                  if (isFuture) {
                    return (
                      <div
                        key={item.dateStr}
                        id={`date-cell-${item.dateStr}`}
                        className="flex items-center justify-center cursor-not-allowed select-none opacity-40"
                        title="未來日期無法選擇或填寫"
                      >
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-normal text-[1.0625rem] text-slate-400">
                          {item.dayNum}
                        </div>
                      </div>
                    );
                    ...
                    >
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-[1.0625rem] transition-all ${
                          // Filled record -> SOLID ORANGE CIRCLE (橘燈)
                          hasRecord && isSelected
                            ? 'bg-[#ea580c] text-white shadow-md shadow-orange-500/40 ring-4 ring-orange-300 scale-105 font-black'
                            : hasRecord && !isSelected
                            ? 'bg-[#ea580c] text-white shadow-sm font-black'
                            : isSelected && !hasRecord
                            ? 'border-2 border-[#ea580c] text-slate-900 font-black shadow-2xs'
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur-xs border-t border-slate-100 space-y-2 z-30 shadow-lg">
        {/* Date Display: e.g. 2026/09/01 */}
        <div className="text-center font-extrabold text-[1.125rem] text-slate-900 tracking-wide">
